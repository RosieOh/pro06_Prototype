...
public class studentEntranceController {
    ...
    private String registerForm(Model model, Principal principal) {
        model.addAttribute("principal", principal);
        String mid = principal.getName();
        Member member = memberRepository.findByMid(mid);
        model.addAttribute("writer", member.getMname());
        return "studententracne/register";
    }


    @PreAuthorize("hasAnyRole('ADMIN', 'USER')")
    @PostMapping("/studententrance/register")
    private String studententranceRegister(@Valid BoardDTO boardDTO, BindingResult bindingResult, RedirectAttributes redirectAttributes) {
        log.info("Student POST register.....");
    ...
    private String modifyForm(Long bno, String boardType, Model model) {
